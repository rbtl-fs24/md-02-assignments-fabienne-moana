...
google: "how to hide outliers in boxplot in R ggplot"

link: <https://www.geeksforgeeks.org/ignore-outliers-in-ggplot2-boxplot-in-r/>

continent is anon-numerical categorical variable

life expectancy is a numerical continuous variable
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    geom_bar counts the rows of the variable continents and plots them as bars. in this case, as we had count as the y-axis for the geom_col function, the plots are the same.
...
the first two numbers describe the range of the x-axis, while the third value sets the number of bars in the plot. the smaller the third number the more bars are displayed, as the third number describes the width of the bars (e.g. for 2.5 ecvery bar has the width of 2.5 years)
